...
This fork runs against Mo's self-hosted backend on a ZimaBoard `mo@192.168.0.35` (hostname `fedora`; SSH key-auth works). Two-container stack at `/home/mo/xiaozhi/`: a locally-built `xiaozhi-esp32-server:zimaboard-local` (xinnan-tech base + a thin overlay baking in `boto3` and `psutil` — see `scripts/Dockerfile-zimaboard-server`) and `litellm` (BerriAI). LLM = `ollama_chat/gemma4:31b-cloud` via LiteLLM → **Ollama Cloud** (set in `LLM.ClaudeViaLiteLLM` — the alias name is historical; the underlying model was swapped Bedrock Haiku 4.5 → Gemma 4 31B in May 2026 for noticeably better multi-turn tool-call discipline; Tau2 76.9% vs Haiku's ~mid-50s). STT FunASR (SenseVoiceSmall), TTS **Amazon Polly** (en-GB neural, default voice `Amy`; earlier setup used EdgeTTS `en-AU-NatashaNeural`). Device reaches the server on LAN port `8003` (OTA/HTTP) and `8000` (WebSocket).
